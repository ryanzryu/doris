...
Status ThriftServer::start() {
    ...
    // add binary_protocal_factory for call TBinaryProtocolFactory's member function:setStringSizeLimit
    std::shared_ptr<apache::thrift::protocol::TBinaryProtocolFactory> binary_protocal_factory = 
            std::dynamic_pointer_cast<apache::thrift::protocol::TBinaryProtocolFactory>(protocol_factory);
    binary_protocal_factory->setStringSizeLimit(config::be_thrift_max_pkg_size);
